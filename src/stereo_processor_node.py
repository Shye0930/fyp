...
class StereoProcessorNode(Node):
    def __init__(self):
        ...
        calibration_file_path =  absolute_path = os.path.abspath(os.path.join(script_dir, '../config/stereo/stereo_calibration.yaml'))
        ...
        try:
            ...
            if os.path.exists(calibration_file_path):
                ...
                self.get_logger().info(f"Loaded YAML file from: {calibration_file_path}")
            else:
                self.get_logger().warn(f"Could not find YAML file at: {calibration_file_path}")
                self.config = {}  # Set empty dict if file not found
        except Exception as e:
            self.get_logger().error(f"Error reading YAML file: {e}")

        self.declare_parameter('calibration_file', calibration_file_path)
