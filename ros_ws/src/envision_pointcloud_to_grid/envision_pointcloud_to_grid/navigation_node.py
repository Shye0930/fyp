...
class NavigationNode(Node):
    ...
    def is_occupied(self, grid_x, grid_y):
        ...
        user_size_m = 0.55  # User width and height in meters
    ...
    def calculate_90_degree_path(self):
        ...
        for i in range(len(checkpoints_grid) - 1):
            ...
            if dist > 0:
                ...
                instruction += f"Walk approximately {num2words(int(dist))} meters"
    ...
    def pose_callback(self, msg):
        ...
        transformed_pose_raw = do_transform_pose(msg.pose, self.transform)
        transformed_pose = PoseStamped()
        transformed_pose.header.stamp = msg.header.stamp
        transformed_pose.header.frame_id = self.transform.header.frame_id
        transformed_pose.pose = transformed_pose_raw
...
#         transformed_pose = do_transform_pose(msg.pose, self.transform)
...
#             distance = math.sqrt(dx**2 + dy**2)
#             self.get_logger().info(f'Distance to goal: {distance:.2f} meters')
#             if distance <= self.goal_radius * 2 and not self.goal_reached:
#                 self.goal_reached = True
#                 goal_reached_msg = Bool()
#                 goal_reached_msg.data = True
#                 self.goal_reached_publisher.publish(goal_reached_msg)
