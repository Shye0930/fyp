...
class StereoCameraPublisher(Node):
    ...
    def __init__(self):
        ...
        # New publisher for the actual publish rate
        self.rate_pub = self.create_publisher(Float64, '/stereo/actual_publish_rate', 10)
        ...
        # Timer for calculating and publishing the actual rate every second
        self.frame_count = 0
        self.last_rate_time = self.get_clock().now()
        self.rate_timer = self.create_timer(1.0, self.publish_current_rate)
    ...
    def publish_stereo_frames(self):
        ...
        if ret_l and ret_r:
            ...
            # Increment frame count for rate calculation
            self.frame_count += 1
    ...
    def publish_current_rate(self):
        current_time = self.get_clock().now()
        time_elapsed = (current_time - self.last_rate_time).nanoseconds / 1e9 # Convert to seconds

        if time_elapsed > 0:
            actual_rate = self.frame_count / time_elapsed
            rate_msg = Float64()
            rate_msg.data = actual_rate
            self.rate_pub.publish(rate_msg)
            self.get_logger().info(f"Actual Stereo Publish Rate: {actual_rate:.2f} Hz")
        else:
            # If no time has elapsed (e.g., first call), or if there was an issue
            self.get_logger().warn("Time elapsed is zero or negative for rate calculation.")
