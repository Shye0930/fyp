...
class StereoProcessorNode(Node):
    def __init__(self):
        ...
        # --- MODIFIED SECTION FOR CALIBRATION FILE DEFAULT ---
        # Resolve the default calibration file path using get_package_share_directory
        # This makes the default robust whether run via launch or directly.
        try:
            package_share_directory = get_package_share_directory('stereo_camera_pipeline')
            default_calibration_file_path = os.path.join(package_share_directory, 'config', 'stereo_calibration.yaml')
            self.declare_parameter('calibration_file', default_calibration_file_path)
        except Exception as e:
            self.get_logger().error(f"Could not determine package share directory for default calibration: {e}")
            self.declare_parameter('calibration_file', '') # Fallback to empty string if lookup fails
        ...
        self.calibration_file = self.get_parameter('calibration_file').get_parameter_value().string_value # This will now be the absolute path

        self.get_logger().info(f"Using Left Camera URL: {self.left_url}")
        self.get_logger().info(f"Using Right Camera URL: {self.right_url}")
        self.get_logger().info(f"Attempting to load calibration from parameter: {self.calibration_file}")
        ...
        self.raw_left_image_pub = self.create_publisher(Image, '/stereo/left/image_raw', 10) # Directly using string
        self.raw_right_image_pub = self.create_publisher(Image, '/stereo/right/image_raw', 10) # Directly using string (and corrected topic)
        ...
        self.pub_rectified_left = self.create_publisher(Image, '/stereo/left/rectified_images', 10) # Directly using string
        self.pub_rectified_right = self.create_publisher(Image, '/stereo/right/rectified_images', 10) # Directly using string
    ...
    def process_and_publish_frames(self):
        ...
        if ret_l and ret_r:
            ...
            if self.M1l is not None and self.M1r is not None:
                ...
                self.get_logger().warn("[WARN] Rectification maps not initialized. Skipping rectification.")
        else:
